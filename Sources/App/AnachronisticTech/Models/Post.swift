...
final class Post: Model, Content {
    ...
    func toOutput(with request: Request) -> Output {
        let markdown = try! Folder(path: "\(request.application.directory.publicDirectory)/AnachronisticTech/content/posts")
            .file(named: "\(id!).md")
        ...
        let imageModifier = Modifier(target: .images) { html, _ in
            ...
            return """
            <figure>
                \(isVideo ? self.video(src: elements[1]) : "<img src=\"/AnachronisticTech/content/images/\(elements[1])\"\(hasCaption ? "alt=\"\(elements[3])\"" : "")>" )
    ...
    private func video(src: String) -> String {
        return """
        <video width="100%" height="auto" controls>
            <source src="/AnachronisticTech/content/images/\(src)" type="video/mp4">
        </video>
        """
    }
